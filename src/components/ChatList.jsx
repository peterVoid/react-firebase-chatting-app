  ...
  useEffect(() => {
    const searchContacts = async () => {
      if (searchUser.trim() === "") {
        ...
        const listing = listContact.filter((item) => {
          if (item.user.username.includes(searchUser)) {
            return item;
          }
        });
        setLisContact(listing);
      }
    };
  ...
  return (
    <div className="p-5">
      ...
      <div className="mt-5 flex flex-col gap-5">
        {!listContact || !listContact.length === 0 ? null : (
          <>
            {listContact?.map((item) => (
              ...
              >
                <img
                  src={item.user ? item.user.profilePicUrl : "./avatar.png"}
                  alt=""
                  className="avatar"
                />
